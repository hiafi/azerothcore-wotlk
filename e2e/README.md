...
| spells/aura | apply/query; CC broken by damage; mount persist; DoT duration is haste-invariant | P1 | covered (`TestAC_26130_*`, `TestDoT_HasteDoesNotShortenCorruptionDuration`); DoT periodic-crit and extra-tick/bonus-tick-damage `blocked-harness` (no SMSG_PERIODIC_AURA_LOG per-tick amount/crit-flag parsing, no GM crit-rating setter — can't force/observe a tick crit or count ticks deterministically) | #26130 |
