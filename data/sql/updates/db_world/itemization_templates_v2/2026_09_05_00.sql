-- DB update 2026_09_04_08 -> 2026_09_05_00
--
...
CREATE TABLE IF NOT EXISTS `item_weapon_dps_curve` (
  `ilvl` smallint unsigned NOT NULL,
  `quality` tinyint unsigned NOT NULL,
  `is_single_slot` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '0=paired (One-Hand/Main Hand/Off Hand, shares its budget with a second item), 1=single-slot (Two-Hand/Ranged/Thrown/Relic, ~1.30x a paired weapon''s DPS at the same ilvl/quality -- see docs/itemization-phase-2.md §7.7)',
  `dps` float NOT NULL,
  PRIMARY KEY (`ilvl`, `quality`, `is_single_slot`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Weapon DPS by item level, quality, and hand category, speed-independent';
