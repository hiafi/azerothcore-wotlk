...
Single-user LAN tool: no auth, no database, no JS build step. Normally served
as part of apps/wow-tools-webui (this module only defines the `dbc` Blueprint;
see that app's app.py for how it's mounted and run) at /dbc/ on whatever host
serves that combined app — see the README's "Web UI" section before exposing
this beyond a trusted home network; there's no login, so anyone who can reach
it can edit these files.

Can still be run standalone for quick dev/debugging:
  python3 apps/dbc-tools/webui/app.py
reachable at http://<this-machine's-LAN-IP>:8600/ (unprefixed, since nothing
...
from flask import Blueprint, Flask, abort, flash, jsonify, redirect, render_template, request, url_for  # noqa: E402
...
bp = Blueprint("dbc", __name__, template_folder="templates", static_folder="static")
...
def index():
    ...
    return render_template("dbc/index.html", spell_files=spell_files, talent_files=talent_files)
...
def run_generate():
    ...
    return render_template("dbc/generate_result.html", result=result)


# -- spells -------------------------------------------------------------


@bp.route("/spells/<file>")
def spell_list(file: str):
    path = spell_csv_path(file)
    entries = sorted(source.load_one_spell_file(path), key=lambda e: e["id"])
    return render_template("dbc/spell_list.html", file=file, entries=entries)


@bp.route("/spells/<file>/new")
def spell_new(file: str):
    ...
    return render_template("dbc/spell_form.html", file=file, original_id="", ctx=ctx, **_spell_form_kwargs(ctx))


@bp.route("/spells/<file>/<int:spell_id>/edit")
def spell_edit(file: str, spell_id: int):
    ...
    return render_template("dbc/spell_form.html", file=file, original_id=spell_id, ctx=ctx, **_spell_form_kwargs(ctx))
...
@bp.route("/api/icon-url/<int:icon_id>")
...
@bp.route("/api/spell-info/<int:spell_id>")
...
@bp.route("/spells/<file>/save", methods=["POST"])
...
@bp.route("/spells/<file>/<int:spell_id>/delete", methods=["POST"])
...
def talent_list(file: str):
    ...
    return render_template(
        "dbc/talent_list.html", file=file,
...
@bp.route("/talents/<file>/tabs/new")
def tab_new(file: str):
    talent_yaml_path(file)
    ctx = view_models.tab_entry_to_form_context({"id": _suggest_id("talenttab")})
    return render_template("dbc/tab_form.html", file=file, original_id="", ctx=ctx)


@bp.route("/talents/<file>/tabs/<int:tab_id>/edit")
def tab_edit(file: str, tab_id: int):
    ...
    return render_template("dbc/tab_form.html", file=file, original_id=tab_id, ctx=ctx)


@bp.route("/talents/<file>/tabs/save", methods=["POST"])
...
@bp.route("/talents/<file>/tabs/<int:tab_id>/delete", methods=["POST"])
...
@bp.route("/talents/<file>/talents/new")
def talent_new(file: str):
    talent_yaml_path(file)
    ctx = view_models.talent_entry_to_form_context({"id": _suggest_id("talent")})
    return render_template(
        "dbc/talent_form.html", file=file, original_id="", ctx=ctx, last_rank_info=None,
    )


@bp.route("/talents/<file>/talents/<int:talent_id>/edit")
def talent_edit(file: str, talent_id: int):
    ...
    return render_template(
        "dbc/talent_form.html", file=file, original_id=talent_id, ctx=ctx,
...
@bp.route("/talents/<file>/talents/save", methods=["POST"])
...
@bp.route("/talents/<file>/talents/<int:talent_id>/delete", methods=["POST"])
...
@bp.route("/talents/<file>/abilities/new")
def ability_new(file: str):
    talent_yaml_path(file)
    ctx = view_models.ability_entry_to_form_context({"id": _suggest_id("skilllineability")})
    return render_template("dbc/skilllineability_form.html", file=file, original_id="", ctx=ctx)


@bp.route("/talents/<file>/abilities/<int:ability_id>/edit")
def ability_edit(file: str, ability_id: int):
    ...
    return render_template("dbc/skilllineability_form.html", file=file, original_id=ability_id, ctx=ctx)


@bp.route("/talents/<file>/abilities/save", methods=["POST"])
...
@bp.route("/talents/<file>/abilities/<int:ability_id>/delete", methods=["POST"])
