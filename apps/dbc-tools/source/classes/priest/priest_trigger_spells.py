...
# Priest Shadow rework (SHADOW.md (0,2) / design doc §7): full retune. eff1 was ADD_PCT_MODIFIER
# SPELLMOD_DAMAGE scoped to a Shadow-spell classmask - now a plain MOD_DAMAGE_PERCENT_DONE (79)
# misc_value=127 (all-school "all damage done", matching the design doc's "Increases all damage
# done by 1/2/3%" - was Shadow-only). eff2 keeps ADD_PCT_MODIFIER (108) but its SpellModOp moves
# from DOT (22) to DAMAGE (0, the default) and its classmask (letter B = effect index 2) narrows to
# Mind Blast only. eff3 is a DUMMY marker holding the proc-chance display value (2/4/6%) - the real
# proc chance is the procs_on() call below; spell_pri_darkness (WP-B) reads presence, not amount.
darkness_15259 = spell(
    ...
    spell_icon_id=164,
    notes='Priest Shadow rework (SHADOW.md (0,2)): eff1 all-damage% (was Shadow-only ADD_PCT_MODIFIER); '
          'eff2 Mind Blast-scoped SPELLMOD_DAMAGE; eff3 DUMMY marker = proc-chance display value.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases all damage done by $s1%. Increases your Mind Blast damage by $s2%. When your Shadow Word: Pain, Mind Flay or Mind Sear deal damage you have a $s3% chance to reset the cooldown of Mind Blast and cause your next Mind Blast to have no cooldown and cost no mana.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
procs_on(darkness_15259, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG | PROC_FLAG_DONE_PERIODIC,
         family_name=6, family_mask=(_masks.SWP | _masks.MIND_FLAY_TICK, _masks.MIND_SEAR_TICK, _masks.MIND_FLAY),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=2)
scripted_by(darkness_15259, 'spell_pri_darkness')
...
shadow_affinity_15272 = spell(
    ...
    range_yards=0.0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-5, implicit_target_a=1, apply_aura=AuraType.MOD_POWER_COST_SCHOOL_PCT, misc_value=32),
    ...
    spell_icon_id=178,
    notes='Priest Shadow rework (SHADOW.md (1,0)): see shadow_affinity_15318 (rank 1) for the full '
          'retune rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Reduces the mana cost of your Shadow spells by $s1%. Increases the duration of your Shadow Word: Pain, Vampiric Touch and Devouring Plague by $/1000;s2 sec.\n\n|cFF9D9D9DCapstone Bonus: All threat generated is reduced by 30%. Does not stack with similar capstone effects.|r', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectSpellClassMaskB_1': _masks.SWP | _masks.DEVOURING_PLAGUE, 'EffectSpellClassMaskB_2': _masks.VT, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)


# Priest Shadow rework (SHADOW.md (2,1) / design doc §7): full retune, now 3 ranks (15314/15316
# orphaned). eff1 cooldown reduction grows -0.5/1/1.5s -> -1/2/3s. eff2 was a plain DUMMY (the stock
# "20% healing reduction" proc, read by a core hardcode script) - REMOVED per design doc §8 and
# replaced with a real ADD_PCT_MODIFIER SPELLMOD_DAMAGE scoped to Mind Blast only. ProcTypeMask=0
# disables the stock DBC-level auto-proc now that there's no proc effect left on this row.
improved_mind_blast_15273 = spell(
    ...
    spell_icon_id=95,
    notes='Priest Shadow rework (SHADOW.md (2,1)): see the block comment above for the full retune.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Reduces the cooldown of your Mind Blast spell by $/1000;s1 sec. Your Mind Blast deals $s2% additional damage.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectSpellClassMaskA_1': _masks.MIND_BLAST, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'ProcTypeMask': 0, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
improved_shadow_word_pain_15275 = spell(
    ...
    spell_icon_id=234,
    notes='Priest Shadow rework (SHADOW.md (1,1)): eff1 retuned 3/6%->5/10% live (stored 4/9); new '
          'eff2 SPELLMOD_COST -10/-20%, scoped to SW:P only (same dword1 SWP bit eff1 already uses).',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the damage of your Shadow Word: Pain spell by $s1% and reduces its mana cost by $s2%.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectSpellClassMaskA_1': _masks.SWP, 'EffectSpellClassMaskB_1': _masks.SWP, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
vampiric_embrace_15286 = spell(
    ...
    spell_icon_id=150,
    notes='Priest Shadow rework (SHADOW.md "Baseline spell edits" / design doc §7 (2,3)): self-heal '
          '15%->10% (stored 14->9, die_sides=1 convention). Party share stays the $/5;15286s1% '
          'client-computed fraction (10/5 = 2%, matching design doc\'s "2%"). Tooltip adds the '
          '5%-of-caster-max-health cap (enforced in spell_pri_vampiric_embrace, WP-B) and the '
          '"Usable while shapeshifted" clause.',
    raw_overrides={'AttributesEx3': 67108864, 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': '$15286s1% of single-target Priest Shadow spell damage caused by casting priest heals the priest and $/5;15286s1% heals the group.', 'BaseLevel': 1, 'CastingTimeIndex': 1, 'DefenseType': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Fills you with the embrace of Shadow energy, causing you to be healed for $15286s1% and other party members to be healed for $/5;15286s1% of any single-target Priest Shadow spell damage you deal for $15286d. Either heal cannot exceed 5% of the maximum health of the caster. Usable while shapeshifted.', 'EffectBonusMultiplier_1': 1.0, 'EffectBonusMultiplier_2': 1.0, 'EffectBonusMultiplier_3': 1.0, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712188, 'Name_Lang_Mask': 16712190, 'PreventionType': 1, 'ProcChance': 100, 'ProcTypeMask': 327680, 'RangeIndex': 1, 'SpellClassMask_1': 4, 'SpellClassSet': 6, 'SpellLevel': 1, 'SpellVisualID_1': 3582, 'StartRecoveryCategory': 133, 'StartRecoveryTime': 1500},
)
scripted_by(vampiric_embrace_15286, 'spell_pri_vampiric_embrace')
...
darkness_15307 = spell(
    ...
    spell_icon_id=164,
    notes='Priest Shadow rework (SHADOW.md (0,2)): see darkness_15259 (rank 1) for the full retune '
          'rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases all damage done by $s1%. Increases your Mind Blast damage by $s2%. When your Shadow Word: Pain, Mind Flay or Mind Sear deal damage you have a $s3% chance to reset the cooldown of Mind Blast and cause your next Mind Blast to have no cooldown and cost no mana.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
procs_on(darkness_15307, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG | PROC_FLAG_DONE_PERIODIC,
         family_name=6, family_mask=(_masks.SWP | _masks.MIND_FLAY_TICK, _masks.MIND_SEAR_TICK, _masks.MIND_FLAY),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=4)
scripted_by(darkness_15307, 'spell_pri_darkness')
...
procs_on(darkness_15308, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG | PROC_FLAG_DONE_PERIODIC,
         family_name=6, family_mask=(_masks.SWP | _masks.MIND_FLAY_TICK, _masks.MIND_SEAR_TICK, _masks.MIND_FLAY),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=6)
scripted_by(darkness_15308, 'spell_pri_darkness')
# darkness_15309/15310 (old ranks 4/5) are now orphaned - Darkness is a 3-rank talent in this tree
# (SHADOW.md (0,2)). Left declared, unreferenced, per repo convention (don't delete pulled data).
...
improved_mind_blast_15312 = spell(
    ...
    spell_icon_id=95,
    notes='Priest Shadow rework (SHADOW.md (2,1)): see improved_mind_blast_15273 (rank 1) for the '
          'full retune rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Reduces the cooldown of your Mind Blast spell by $/1000;s1 sec. Your Mind Blast deals $s2% additional damage.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectSpellClassMaskA_1': _masks.MIND_BLAST, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'ProcTypeMask': 0, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
# improved_mind_blast_15314/15316 (old ranks 4/5) are now orphaned - Improved Mind Blast is a
# 3-rank talent in this tree (SHADOW.md (2,1)). Left declared, unreferenced.
...
improved_shadow_word_pain_15317 = spell(
    ...
    spell_icon_id=234,
    notes='Priest Shadow rework (SHADOW.md (1,1)): see improved_shadow_word_pain_15275 (rank 1) for '
          'the full retune rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the damage of your Shadow Word: Pain spell by $s1% and reduces its mana cost by $s2%.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectSpellClassMaskA_1': _masks.SWP, 'EffectSpellClassMaskB_1': _masks.SWP, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)


# Priest Shadow rework (SHADOW.md (1,0) / design doc §7): full retune. eff1 was ADD_PCT_MODIFIER
# SPELLMOD_THREAT (dropped) - now MOD_POWER_COST_SCHOOL_PCT (72) misc=32 (Shadow), a real mana-cost
# reduction. eff2 was a DUMMY dispel-mana-return marker (dropped, per design doc §8 "clauses
# removed") - now ADD_FLAT_MODIFIER SPELLMOD_DURATION (misc=1) scoped to SW:P|Devouring Plague
# (dword 1) and VT (dword 2), extending their duration by a flat 1/2/3 sec. r3 only: capstone eff3
# MOD_THREAT (10) misc=127 (all-school) -30% flat threat reduction (PLAN §3.1 tooltip convention).
shadow_affinity_15318 = spell(
    ...
    range_yards=0.0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-3, implicit_target_a=1, apply_aura=AuraType.MOD_POWER_COST_SCHOOL_PCT, misc_value=32),
    ...
    spell_icon_id=178,
    notes='Priest Shadow rework (SHADOW.md (1,0)): see the block comment above for the full retune.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Reduces the mana cost of your Shadow spells by $s1%. Increases the duration of your Shadow Word: Pain, Vampiric Touch and Devouring Plague by $/1000;s2 sec.\n\n|cFF9D9D9DCapstone Bonus: All threat generated is reduced by 30%. Does not stack with similar capstone effects.|r', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectSpellClassMaskB_1': _masks.SWP | _masks.DEVOURING_PLAGUE, 'EffectSpellClassMaskB_2': _masks.VT, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
shadow_affinity_15320 = spell(
    ...
    range_yards=0.0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-7, implicit_target_a=1, apply_aura=AuraType.MOD_POWER_COST_SCHOOL_PCT, misc_value=32),
    ...
    spell_icon_id=178,
    notes='Priest Shadow rework (SHADOW.md (1,0)): see shadow_affinity_15318 (rank 1) for the full '
          'retune rationale. Final rank only: capstone eff3 (MOD_THREAT, all-school -30%) - the '
          "tooltip's capstone clause is plain/full-color on this rank (PLAN §3.1).",
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Reduces the mana cost of your Shadow spells by $s1%. Increases the duration of your Shadow Word: Pain, Vampiric Touch and Devouring Plague by $/1000;s2 sec.\n\nCapstone Bonus: All threat generated is reduced by 30%. Does not stack with similar capstone effects.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskB_1': _masks.SWP | _masks.DEVOURING_PLAGUE, 'EffectSpellClassMaskB_2': _masks.VT, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
# Priest Shadow rework (SHADOW.md (0,0)/(5,2)): orphaned - talent 2027 (old Improved Spirit Tap)
# is fully repurposed into Insatiable Thirst at (5,2); its mechanism was folded into the merged
# Spirit Tap (0,0) instead (spirit_tap_15270/15335/15336 above). Left declared, unreferenced, per
# repo convention. Their own trigger_spell targets (improved_spirit_tap_49694, bare-int 59000
# referenced by 15338 below) become orphaned transitively - not touched.
...
# Priest Shadow rework (SHADOW.md (3,2) / design doc §7): eff1 moves from a %-range modifier to a
# flat-yard ADD_FLAT_MODIFIER SPELLMOD_RANGE (same classmask, unchanged).
#
# The Corrupted Soul (200247) proc is a **Capstone Bonus**: max rank only, at a flat 10% (it was
# briefly a both-ranks 5/10% proc - user correction 2026-09-23). So only rank 2 (17323) carries
# procs_on/scripted_by, exactly like the 2-rank Holy Reach capstone (27789/27790). Both ranks
# still *describe* the capstone, with rank 1 greying it out via |cFF9D9D9D...|r, and its flat 10%
# is written into the tooltip text rather than driven by a $s2 effect - both per that precedent,
# which is why the old eff2 DUMMY proc-chance display marker is gone.
shadow_reach_17322 = spell(
    ...
    spell_icon_id=55,
    notes='Priest Shadow rework (SHADOW.md (3,2)): see the block comment above for the full retune.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the range of your damaging spells with a channeling or cast time by $s1 yards.\n\n|cFF9D9D9DCapstone Bonus: This does not stack with other similar effects. Dealing direct magic damage to a target further than 20 yards away from you has a 10% chance to corrupt their soul, dealing Shadow damage over 3 seconds. This effect can only occur once every 5 seconds.|r', 'EffectBonusMultiplier_1': 1.0, 'EffectBonusMultiplier_2': 1.0, 'EffectBonusMultiplier_3': 1.0, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskA_1': 109223940, 'EffectSpellClassMaskA_2': 3147010, 'EffectSpellClassMaskA_3': 8256, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
shadow_reach_17323 = spell(
    ...
    spell_icon_id=55,
    notes='Priest Shadow rework (SHADOW.md (3,2)): see shadow_reach_17322 (rank 1) for the full '
          'retune rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the range of your damaging spells with a channeling or cast time by $s1 yards.\n\nCapstone Bonus: This does not stack with other similar effects. Dealing direct magic damage to a target further than 20 yards away from you has a 10% chance to corrupt their soul, dealing Shadow damage over 3 seconds. This effect can only occur once every 5 seconds.', 'EffectBonusMultiplier_1': 1.0, 'EffectBonusMultiplier_2': 1.0, 'EffectBonusMultiplier_3': 1.0, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskA_1': 109223940, 'EffectSpellClassMaskA_2': 3147010, 'EffectSpellClassMaskA_3': 8256, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
procs_on(shadow_reach_17323, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG,
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=10, cooldown_ms=5000)
scripted_by(shadow_reach_17323, 'spell_pri_shadow_reach_capstone')
...
# Priest Shadow rework (SHADOW.md (9,1), MOVED from (7,2) / design doc §7): eff1 (PROC_TRIGGER_SPELL
# -> the stock 33196-33198 spell-hit-chance debuff) removed - ProcTypeMask=0 disables the stock
# auto-proc entirely. eff3 (letter C, unchanged slot) retuned 5/10/15% -> 8/16/25% (stored 7/15/24).
misery_33191 = spell(
    id=33191,
    name='Misery',
    school=School.NORMAL,
    ...
    spell_icon_id=2211,
    notes='Priest Shadow rework (SHADOW.md (9,1)): see the block comment above for the full retune.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the spell damage scaling of your Mind Blast, Mind Flay and Vampiric Touch spells by $s3%.', 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskC_1': 8396800, 'EffectSpellClassMaskC_2': 524288, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 100, 'ProcTypeMask': 0, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
misery_33192 = spell(
    ...
    spell_icon_id=2211,
    notes='Priest Shadow rework (SHADOW.md (9,1)): see misery_33191 (rank 1) for the full retune '
          'rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the spell damage scaling of your Mind Blast, Mind Flay and Vampiric Touch spells by $s3%.', 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskC_1': 8396800, 'EffectSpellClassMaskC_2': 524288, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 100, 'ProcTypeMask': 0, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
# misery_33196/33197/33198 (the old PROC_TRIGGER_SPELL targets) are now orphaned. Left declared,
# unreferenced, per repo convention (don't delete pulled data).
...
# Priest Shadow rework (SHADOW.md (6,2) / design doc §7): now 3 ranks (33224/33225 orphaned).
# eff2 (letter B, unchanged slot) retuned 20/40/60% -> 30/60/100% crit-damage bonus.
...
# this tree (SHADOW.md (6,2)). Left declared, unreferenced.
...
# Priest Shadow rework (SHADOW.md (9,2) / design doc §7): now 3 ranks (51166/51167 orphaned).
# eff1 (aura 174, Spirit->SP, unchanged slot) retuned 4/8/12% -> 8/16/25% (stored 7/15/24). eff2
# (OVERRIDE_CLASS_SCRIPTS misc 7377, the Mind Flay/Mind Blast-vs-SWP-target damage bonus the core
# reads for both) retuned 2/4/6% -> 3/6/10% (stored 2/5/9). eff3 (aura 175, Spirit->healing,
# unchanged slot) kept mirroring eff1's value for symmetry with the stock row's own shape.
twisted_faith_47573 = spell(
    ...
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=7, implicit_target_a=1, apply_aura=174, misc_value=126),
        Effect(type=EffectType.APPLY_AURA, base_points=2, implicit_target_a=1, apply_aura=112, misc_value=7377),
        Effect(type=EffectType.APPLY_AURA, base_points=7, implicit_target_a=1, apply_aura=175, misc_value=4),
    ],
    spell_icon_id=2848,
    notes='Priest Shadow rework (SHADOW.md (9,2)): see the block comment above for the full retune.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases your spell power by $s1% of your total Spirit, and your damage done by Mind Flay and Mind Blast is increased by $s2% if your target is afflicted by your Shadow Word: Pain.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectMiscValueB_1': 4, 'EffectMiscValueB_3': 4, 'EffectSpellClassMaskA_1': 1026, 'EffectSpellClassMaskA_2': 1, 'EffectSpellClassMaskB_1': 8192, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
twisted_faith_47577 = spell(
    ...
    spell_icon_id=2848,
    notes='Priest Shadow rework (SHADOW.md (9,2)): see twisted_faith_47573 (rank 1) for the full '
          'retune rationale.',
...
# twisted_faith_51166/51167 (old ranks 4/5) are now orphaned - Twisted Faith is a 3-rank talent in
# this tree (SHADOW.md (9,2)). Left declared, unreferenced.
...
pain_and_suffering_47580 = spell(
    ...
    spell_icon_id=2874,
    notes='pulled from existing data',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Mind Flay, Mind Sear and Mind Blast casts have a $h% chance to refresh the duration of your Shadow Word: Pain on the target. Reduces the damage you take from your own Shadow Word: Death by $s2%.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskB_1': 2, 'EffectSpellClassMaskB_2': 32785, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 33, 'ProcTypeMask': 65536, 'RangeIndex': 1, 'SpellClassSet': 6},
)
# Priest Shadow rework (SHADOW.md (8,1)): rebuilt as a cast-phase proc (was damage-phase via the
# stock DBC ProcTypeMask alone) so it fires once per Mind Flay/Mind Sear/Mind Blast cast rather than
# per tick - matches design doc §7's "casts have a % chance". Keep the SW:D reduction effect as-is.
procs_on(pain_and_suffering_47580, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, spell_phase_mask=PROC_SPELL_PHASE_CAST,
         family_name=6, family_mask=(_masks.MIND_BLAST, _masks.MIND_SEAR, _masks.MIND_FLAY), chance=33)
...
pain_and_suffering_47581 = spell(
    ...
    spell_icon_id=2874,
    notes='pulled from existing data',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Mind Flay, Mind Sear and Mind Blast casts have a $h% chance to refresh the duration of your Shadow Word: Pain on the target. Reduces the damage you take from your own Shadow Word: Death by $s2%.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskB_1': 2, 'EffectSpellClassMaskB_2': 32785, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 66, 'ProcTypeMask': 65536, 'RangeIndex': 1, 'SpellClassMask_1': 8388608, 'SpellClassSet': 6},
)
procs_on(pain_and_suffering_47581, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, spell_phase_mask=PROC_SPELL_PHASE_CAST,
         family_name=6, family_mask=(_masks.MIND_BLAST, _masks.MIND_SEAR, _masks.MIND_FLAY), chance=66)
...
pain_and_suffering_47582 = spell(
    ...
    spell_icon_id=2874,
    notes='pulled from existing data',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Mind Flay, Mind Sear and Mind Blast casts have a $h% chance to refresh the duration of your Shadow Word: Pain on the target. Reduces the damage you take from your own Shadow Word: Death by $s2%.', 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskB_1': 2, 'EffectSpellClassMaskB_2': 32785, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 100, 'ProcTypeMask': 65536, 'RangeIndex': 1, 'SpellClassSet': 6},
)
procs_on(pain_and_suffering_47582, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, spell_phase_mask=PROC_SPELL_PHASE_CAST,
         family_name=6, family_mask=(_masks.MIND_BLAST, _masks.MIND_SEAR, _masks.MIND_FLAY), chance=100)
# Verify (WP-C): spell_pri_pain_and_suffering_proc (47948) still refreshes SW:P on the proc target
# for a cast-phase proc - the stock binding fired on damage-phase ticks, this rebuild fires once per
# cast instead, so the AuraScript's target-selection logic (which target's SW:P to refresh on a
# multi-target cast like Mind Sear) needs a WP-B re-check.
...
procs_on(empowered_renew_63543, PROC_FLAG_DONE_PERIODIC, hit_mask=PROC_HIT_CRITICAL,
         family_name=6, family_mask=(_masks.RENEW, 0, 0),
...
# Priest Shadow rework (SHADOW.md (0,1), MOVED from (5,2) / design doc §7): eff1 (dot%) unchanged
# 5/10/15%. eff2 (instant-chunk %, read by the 63675 hardcode-migration path) now stored 29 (30%)
# on ALL THREE ranks (was 10/20/30% scaling) - "Devouring Plague now instantly deals 30% of its
# total periodic effect when cast" is a flat number, not rank-scaled, per design doc §7 (0,1). Final
# rank only: new eff3 DUMMY capstone marker (5) - Priest::ApplyDoneDamagePctMods reads presence of
# 63627 on the caster, not this value, to grant +5% Priest-spell damage to DP-afflicted targets.
improved_devouring_plague_63625 = spell(
    ...
    spell_icon_id=3790,
    notes='Priest Shadow rework (SHADOW.md (0,1)): see the block comment above for the full retune.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the periodic damage done by your Devouring Plague by $s1%. Devouring Plague now instantly deals 30% of its total periodic effect when cast.\n\n|cFF9D9D9DCapstone Bonus: All your Priest spells deal 5% increased damage to a target afflicted by your Devouring Plague.|r', 'EffectBonusMultiplier_3': 1.0, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskA_1': 33554432, 'EffectSpellClassMaskB_1': 33554432, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
improved_devouring_plague_63626 = spell(
    ...
    spell_icon_id=3790,
    notes='Priest Shadow rework (SHADOW.md (0,1)): see improved_devouring_plague_63625 (rank 1) for '
          'the full retune rationale.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the periodic damage done by your Devouring Plague by $s1%. Devouring Plague now instantly deals 30% of its total periodic effect when cast.\n\n|cFF9D9D9DCapstone Bonus: All your Priest spells deal 5% increased damage to a target afflicted by your Devouring Plague.|r', 'EffectBonusMultiplier_3': 1.0, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskA_1': 33554432, 'EffectSpellClassMaskB_1': 33554432, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
improved_devouring_plague_63627 = spell(
    ...
    spell_icon_id=3790,
    notes='Priest Shadow rework (SHADOW.md (0,1)): see improved_devouring_plague_63625 (rank 1) for '
          'the full retune rationale. Final rank only: capstone eff3 marker.',
    raw_overrides={'AuraDescription_Lang_Mask': 16712188, 'CastingTimeIndex': 1, 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the periodic damage done by your Devouring Plague by $s1%. Devouring Plague now instantly deals 30% of its total periodic effect when cast.\n\nCapstone Bonus: All your Priest spells deal 5% increased damage to a target afflicted by your Devouring Plague.', 'EffectBonusMultiplier_3': 1.0, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectSpellClassMaskA_1': 33554432, 'EffectSpellClassMaskB_1': 33554432, 'EquippedItemClass': -1, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Name_Lang_Mask': 16712190, 'ProcChance': 101, 'RangeIndex': 1, 'SpellClassSet': 6},
)
...
surge_of_light_33151 = spell(
    ...
    notes='Priest Holy rework (HOLY.md 5,0): design doc §5.7 - "the free cast can now crit and '
          ...
          'PLAN §2\'s "applied unconditionally" (no PvP-only clause) call. CumulativeAura=2, '
...
procs_on(divine_touch_200174, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.RENEW, 0, 0),
         spell_type_mask=PROC_SPELL_TYPE_HEAL, spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=2, cooldown_ms=5000)
...
procs_on(divine_touch_200175, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.RENEW, 0, 0),
         spell_type_mask=PROC_SPELL_TYPE_HEAL, spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=4, cooldown_ms=5000)
...
procs_on(kindled_faith_200187, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, family_name=6,
         family_mask=(_masks.SMITE, 0, 0), spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=8)
...
procs_on(kindled_faith_200188, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, family_name=6,
         family_mask=(_masks.SMITE, 0, 0), spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=16)
...
procs_on(kindled_faith_200189, PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG, family_name=6,
         family_mask=(_masks.SMITE, 0, 0), spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=25)
...
blessed_warding_buff_200206 = spell(
    ...
    notes='HOLY.md 6,2: the Blessed Warding buff - healing done % (BasePoints set by script per rank, '
          'CastCustomSpell), CumulativeAura=3, 10 s.',
    raw_overrides={'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases healing done by $s1%.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Healing done increased by $s1%.', 'EquippedItemClass': -1, 'ProcChance': 101, 'SpellPriority': 50, 'CumulativeAura': 3, 'EffectChainAmplitude_1': 1.0},
)
...
holy_wrath_crit_200213 = spell(
    ...
    notes='HOLY.md 7,3: capstone clause 1 buff - +4% crit chance to Holy-school priest spells '
          '(base_points=3), CumulativeAura=5, ProcCharges=1 (each stack consumes one charge of the '
...
procs_on(epiphany_of_light_200220, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.HOLY_FIRE, 0, 0),
         spell_type_mask=PROC_SPELL_TYPE_DAMAGE, spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=2, cooldown_ms=5000)
...
procs_on(epiphany_of_light_200221, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.HOLY_FIRE, 0, 0),
         spell_type_mask=PROC_SPELL_TYPE_DAMAGE, spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=4, cooldown_ms=5000)
...
procs_on(epiphany_of_light_200222, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.HOLY_FIRE, 0, 0),
         spell_type_mask=PROC_SPELL_TYPE_DAMAGE, spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=6, cooldown_ms=5000)
...
# --- Priest Shadow rework (docs/reworks/priest-shadow-rework.md, priest-rework.SHADOW.md) -------
# WP-0 prep: rows pulled in via `pull_dsl.py 15270 15335 15336 15271 47948 63675 --constants`
# (priest-rework.SHADOW.md "WP-0 prep" step 1) - previously bare-int `ranks=[...]` entries in
# priest_talents.py (Spirit Tap) or npc.csv legacy rows (Spirit Tap's 15271 buff, Pain and
# Suffering's 47948 proc target, Improved Devouring Plague's 63675 instant-chunk spell); the
# matching npc.csv rows were deleted per pull_dsl.py's own printed note (an ID declared in both is
# a generate.py hard error). Pasted verbatim from pull_dsl.py's output - WP-A retunes these in
# place for the rework (Spirit Tap merge with Improved Spirit Tap, etc; see SHADOW.md's talent
# table 0,0).
# Priest Shadow rework (SHADOW.md (0,0)): Spirit Tap MERGED with Improved Spirit Tap. eff1's
# PROC_TRIGGER_SPELL target moves from the old on-kill buff (15271) to a new rank-scaled buff
# (200240/241/242, declared later in this file) that also carries the old Improved Spirit Tap
# mechanism (crit rating from Spirit). ProcTypeMask=0 disables the stock "on kill" DBC-level
# auto-proc entirely - procs_on() below is what the engine actually uses now (crit direct-Shadow-
# damage or Mind Flay damage, chance=100 on every rank; only the buff's own magnitude scales by rank).
spirit_tap_15270 = spell(
    id=15270,
    name='Spirit Tap',
    school=School.NORMAL,
    attributes=464,
...
# granted_by_talent references this id anymore (talent 2027, old Improved Spirit Tap, is fully
# repurposed into Insatiable Thirst at (5,2)). Left declared, unreferenced, per repo convention.
spirit_tap_15271 = spell(
    id=15271,
    name='Spirit Tap',
    school=School.SHADOW,
    dispel=DispelType.MAGIC,
    ...
    duration_ms=15000,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=99, implicit_target_a=1, apply_aura=AuraType.MOD_TOTAL_STAT_PERCENTAGE, misc_value=4),
        Effect(type=EffectType.APPLY_AURA, base_points=82, implicit_target_a=1, apply_aura=AuraType.MOD_MANA_REGEN_INTERRUPT),
    ],
    spell_icon_id=152,
    notes='pulled from existing data',
    raw_overrides={'CastingTimeIndex': 1, 'ProcChance': 101, 'RangeIndex': 1, 'EquippedItemClass': -1, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': 'Rank 1', 'NameSubtext_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Spirit increased by $s1% and allows $s2% of mana regeneration while casting.', 'Description_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Spirit increased by $s1% and allows $s2% of mana regeneration while casting.', 'AuraDescription_Lang_Mask': 16712190, 'SpellClassSet': 6, 'SpellClassMask_2': 4096, 'SpellClassMask_3': 1024, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0, 'EffectBonusMultiplier_2': 1.0, 'EffectBonusMultiplier_3': 1.0},
)


pain_and_suffering_47948 = spell(
    id=47948,
    name='Pain and Suffering',
    school=School.NORMAL,
    cast_time_ms=0,
    ...
    duration_ms=-1,
    effects=[
        Effect(type=77, implicit_target_a=6),
    ],
    spell_icon_id=2874,
    notes='pulled from existing data',
    raw_overrides={'AttributesEx2': 4, 'AttributesEx3': 131072, 'CastingTimeIndex': 1, 'ProcChance': 101, 'EquippedItemClass': -1, 'EffectDieSides_2': 1, 'EffectBasePoints_2': -1, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712172, 'Description_Lang_Mask': 16712188, 'AuraDescription_Lang_Mask': 16712188, 'SpellClassSet': 6, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


improved_devouring_plague_63675 = spell(
    id=63675,
    name='Improved Devouring Plague',
    school=School.SHADOW,
    dispel=DispelType.MAGIC,
    ...
    spell_icon_id=3790,
    notes='pulled from existing data',
    raw_overrides={'AttributesEx2': 524288, 'ShapeshiftMask': 134217728, 'CastingTimeIndex': 1, 'ProcChance': 101, 'BaseLevel': 20, 'SpellLevel': 20, 'EquippedItemClass': -1, 'EffectDieSides_2': 1, 'EffectBasePoints_2': -1, 'EffectMultipleValue_1': 1.0, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712188, 'Description_Lang_enUS': 'Increases the periodic damage done by your Devouring Plague, and when you cast Devouring Plague you instantly deal damage equal to a portion of its total periodic effect.', 'Description_Lang_Mask': 16712190, 'AuraDescription_Lang_Mask': 16712188, 'StartRecoveryCategory': 133, 'StartRecoveryTime': 1500, 'SpellClassSet': 6, 'SpellClassMask_3': 8, 'DefenseType': 1, 'PreventionType': 1, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


# --- Priest Shadow rework: new spells, ID block 200240-200272 (SHADOW.md's "ID map") -------------
# Trigger-only half of the block (never directly cast by the player) - Call of the Void (200248)
# and Surrender to Madness (200269) are player_castable=True and live in priest_spells.py instead,
# per source/classes/README.md's player-castable heuristic.

spirit_tap_buff_200240 = spell(
    id=200240,
    name='Spirit Tap',
    school=School.SHADOW,
    dispel=DispelType.MAGIC,
    ...
    effects=[
        ...
        Effect(type=EffectType.APPLY_AURA, base_points=4, implicit_target_a=1, apply_aura=AuraType.MOD_RATING_FROM_STAT, misc_value=1792),
    ],
    spell_icon_id=152,
    notes='Priest Shadow rework (SHADOW.md (0,0)): Spirit Tap buff rank 1 - +10% Spirit (misc 4), '
          '+25% mana regen while casting, +5% of Spirit as all-crit rating (misc 1792 - PLAN §1\'s '
          'generalized-crit convention; EffectMiscValueB_3=4 selects the Spirit stat for effect 3, '
          'same "EffectMiscValueB_N selects the stat MOD_RATING_FROM_STAT reads" pattern meditation_'
          '14777/mental_strength_18551 already use in this file). 15 s, refreshed on each proc.',
    raw_overrides={'EffectMiscValueB_3': 4, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Spirit increased by $s1%. Mana regeneration while casting allowed at $s2%. Spell critical strike rating increased by $s3% of Spirit.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Spirit and crit rating increased, mana regenerates while casting.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


spirit_tap_buff_200241 = spell(
    id=200241,
    name='Spirit Tap',
    school=School.SHADOW,
    dispel=DispelType.MAGIC,
    ...
    effects=[
        ...
        Effect(type=EffectType.APPLY_AURA, base_points=49, implicit_target_a=1, apply_aura=AuraType.MOD_MANA_REGEN_INTERRUPT),
        Effect(type=EffectType.APPLY_AURA, base_points=9, implicit_target_a=1, apply_aura=AuraType.MOD_RATING_FROM_STAT, misc_value=1792),
    ],
    spell_icon_id=152,
    notes='Priest Shadow rework (SHADOW.md (0,0)): Spirit Tap buff rank 2 - see spirit_tap_buff_200240.',
    raw_overrides={'EffectMiscValueB_3': 4, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Spirit increased by $s1%. Mana regeneration while casting allowed at $s2%. Spell critical strike rating increased by $s3% of Spirit.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Spirit and crit rating increased, mana regenerates while casting.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


spirit_tap_buff_200242 = spell(
    id=200242,
    name='Spirit Tap',
    school=School.SHADOW,
    dispel=DispelType.MAGIC,
    ...
    spell_icon_id=152,
    notes='Priest Shadow rework (SHADOW.md (0,0)): Spirit Tap buff rank 3 - see spirit_tap_buff_200240.',
    raw_overrides={'EffectMiscValueB_3': 4, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Spirit increased by $s1%. Mana regeneration while casting allowed at $s2%. Spell critical strike rating increased by $s3% of Spirit.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Spirit and crit rating increased, mana regenerates while casting.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


darkness_buff_200243 = spell(
    id=200243,
    name='Darkness',
    school=School.SHADOW,
    cast_time_ms=0,
    ...
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-101, implicit_target_a=1, apply_aura=AuraType.ADD_PCT_MODIFIER, misc_value=SpellModOp.COST),
        Effect(type=EffectType.APPLY_AURA, base_points=-101, implicit_target_a=1, apply_aura=AuraType.ADD_PCT_MODIFIER, misc_value=SpellModOp.COOLDOWN),
    ],
    spell_icon_id=164,
    notes="Priest Shadow rework (SHADOW.md (0,2)): Darkness's own proc buff - next Mind Blast costs "
          'no mana (SPELLMOD_COST -100%) and has no cooldown (SPELLMOD_COOLDOWN -100%), both scoped '
          'to Mind Blast only. ProcCharges=1 - consumed on the next matching cast (stock SpellMod '
          'charge-consumption mechanic). Cast by spell_pri_darkness (WP-B) on Darkness\'s own proc.',
    raw_overrides={'ProcCharges': 1, 'SpellClassSet': 6, 'EffectSpellClassMaskA_1': _masks.MIND_BLAST, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your next Mind Blast costs no mana and has no cooldown.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Next Mind Blast free and off cooldown.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


tentacles_of_madness_200244 = spell(
    id=200244,
    name='Tentacles of Madness',
    school=School.NORMAL,
    attributes=448,
    cast_time_ms=0,
    effects=[
    ...
    spell_icon_id=2296,
    notes='Priest Shadow rework (SHADOW.md (3,0)): passive marker for the new talent (repurposes 463, '
          'was Shadow Focus at 1,2). base_points=15 (literal, die_sides=0) mirrors the 15% Mind Blast '
          'spawn chance WP-B hardcodes in spell_pri_mind_blast_shadow - not read by the script, just a '
          "documentation echo of the tooltip's own number.",
    raw_overrides={'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Teaches Tentacle of Madness. Your Mind Blast has a 15% chance to summon one, and always summons one if none has been summoned this way in the last 25 sec.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)


summon_tentacle_of_madness_200245 = spell(
    id=200245,
    name='Summon Tentacle of Madness',
    school=School.SHADOW,
    cast_time_ms=0,
    duration_ms=10000,
    effects=[
        Effect(type=EffectType.SUMMON, implicit_target_a=1, misc_value=300102),
    ],
    spell_icon_id=2296,
    notes='Priest Shadow rework (SHADOW.md (3,0) / ID map): guardian summon, cast by TrySummonTentacle '
          '(PriestMechanics, WP-B). EffectMiscValue_1=300102 (creature entry, WP-0\'s hand-written '
          'pending_db_world migration). EffectMiscValueB_1=1021 (SummonPropertiesEntry - Mirror '
          "Image/Summon Gargoyle's own owner-linked-guardian-no-pet-bar id, pre-verified during "
          'planning per PLAN/SHADOW.md - not independently verifiable through this pipeline, used '
          'verbatim). 10 s duration; npc_pri_tentacle_of_madness (WP-B) manages targeting/despawn.',
    raw_overrides={'EffectMiscValueB_1': 1021, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Summons a Tentacle of Madness.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)


tentacle_mind_flay_200246 = spell(
    id=200246,
    name='Mind Flay',
    school=School.SHADOW,
    attributes=65536,
    ...
    spell_icon_id=548,
    notes="Priest Shadow rework (SHADOW.md (3,0) / ID map): the tentacle's own channeled Mind Flay "
          "clone. Deliberately NOT a copy of stock Mind Flay 15407's exact effect shape (which ticks "
          'via a PERIODIC_TRIGGER_SPELL_WITH_VALUE-type effect that calls a shared, family-less '
          'helper spell, 58381) - that would prevent Lash of Insanity\'s SPELLMOD_DAMAGE from ever '
          'matching on the dword3 MIND_FLAY bit, since 58381 carries its Mind Flay bit in dword1 '
          '(0x800000) instead. (Lash is now scoped to BOTH dwords so it reaches the priest\'s own '
          'Mind Flay too - see lash_of_insanity_200253 - but this row keeping a real PERIODIC_DAMAGE '
          'effect of its own is still what lets the tentacle be modified independently.) '
          'range_yards=40 rather than the RangeIndex 1 the other new rows here carry: those are all '
          'self-cast or triggered, whereas npc_pri_tentacle_of_madness DoCasts this non-triggered at '
          'up to its own 40 yd target-search radius. (Spell::CheckRange short-circuits on '
          'RangeEntry ID 1, so RangeIndex 1 would not actually break the cast today - but a 40 yd '
          'channel should not be leaning on that engine special case.) '
          'Instead this row carries its own real PERIODIC_DAMAGE effect with '
          'SpellClassSet=6 and SpellClassMask_3=MIND_FLAY (the real Mind Flay bit, deliberately '
          'shared - PLAN §4.4), so modOwner->ApplySpellMod(200246, ...) resolves against the owner\'s '
          'Mind-Flay-scoped talents via GetSpellModOwner(). base_points=0 (die_sides=0, fully '
          'coefficient-driven) + bonus_coefficients(dot=0.257) below, matching stock Mind Flay\'s own '
          'live per-tick coefficient (58381\'s spell_bonus_data row, data/sql/base/db_world/'
          'spell_bonus_data.sql: "(58381,0.257,0,0,0,\'Priest - Mind Flay\')" - LEAST CONFIDENT part '
          'of this row: stock Mind Flay 15407 also carries its own EffectBonusMultiplier_3=0.271 on '
          'its DBC row, a different number through a different mechanism (SpellInfo::Effects[i]. '
          'BonusMultiplier, Unit.cpp:8661/9450) that this row does NOT replicate - flagged for review. '
          'No snare effect (design doc §4.1: the tentacle only channels damage, no slow). No mana '
          'cost. AttributesEx |= SPELL_ATTR1_NO_THREAT (0x400) on top of stock Mind Flay\'s own '
          ...
          'for the 1-Madness-per-tick generation.',
...
bonus_coefficients(tentacle_mind_flay_200246, dot=0.257)
scripted_by(tentacle_mind_flay_200246, 'spell_pri_tentacle_mind_flay')


corrupted_soul_200247 = spell(
    id=200247,
    name='Corrupted Soul',
    school=School.SHADOW,
    ...
    notes='Priest Shadow rework (SHADOW.md (3,2) / ID map): Shadow Reach\'s DoT. base_points=0 - '
          'spell_pri_shadow_reach_capstone (WP-B) casts this via CastCustomSpell/SPELLVALUE_BASE_POINT0 '
          '(BP0 = triggering damage x 0.2 / 3, PLAN §2 default), so the DBC row carries no static '
          'amount of its own. 3 ticks over 3 s (amplitude 1000). implicit_target_a=6 '
          '(TARGET_UNIT_TARGET_ENEMY), not 1 (TARGET_UNIT_CASTER) as this row originally shipped - '
          "with caster targeting the engine ignores the explicit target spell_pri_shadow_reach_capstone "
          'passes to CastCustomSpell and the DoT lands on the priest instead of the enemy.',
    raw_overrides={'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Corrupts the soul, dealing Shadow damage over time.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)


call_of_the_void_buff_200249 = spell(
    id=200249,
    ...
    spell_icon_id=90104,
    notes='Priest Shadow rework (SHADOW.md (4,0) / ID map): Call of the Void\'s own marker buff. '
          'base_points=0 - spell_pri_call_of_the_void (WP-B) sets the amount via CastCustomSpell/'
          'SPELLVALUE_BASE_POINT0 (consumed Madness / 2, 0.5% per Madness). The tentacle damage path '
          "reads this buff's live amount directly off the owner (Priest::ApplyDoneDamagePctMods), so "
          'tentacles summoned during the 15 s window pick it up automatically.',
    raw_overrides={'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': "Increases your Tentacles of Madness' damage.", 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': "Tentacle of Madness damage increased.", 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)


deathspeaker_200250 = spell(
    id=200250,
    name='Deathspeaker',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=1980,
    notes='Priest Shadow rework (SHADOW.md (4,1) / ID map): NEW talent, repurposes 1777 (was Focused '
          'Mind at 4,3). eff1 SPELLMOD_DAMAGE scoped to Shadow Word: Death (dword2 SWD) +10/20/30% '
          '(letter A, dword 2). eff2 DUMMY = backlash-tentacle roll chance 15/30/45% (subject to the '
          'shared ICD). eff3 DUMMY = kill-tentacle roll chance 33/66/100% (no ICD). Both rolls live in '
          'spell_pri_shadow_word_death/Priest::OnKill (WP-B); mutually exclusive per cast by '
          "construction (a kill produces no backlash).",
    raw_overrides={'EffectSpellClassMaskA_2': _masks.SWD, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Shadow Word: Death deals $s1% additional damage. When you take damage from your own Shadow Word: Death, you have a $s2% chance to summon a Tentacle of Madness. When your Shadow Word: Death kills an enemy, you have a $s3% chance to summon a Tentacle of Madness.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


deathspeaker_200251 = spell(
    id=200251,
    name='Deathspeaker',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=1980,
    notes='Priest Shadow rework (SHADOW.md (4,1)): Deathspeaker rank 2 - see deathspeaker_200250.',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.SWD, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Shadow Word: Death deals $s1% additional damage. When you take damage from your own Shadow Word: Death, you have a $s2% chance to summon a Tentacle of Madness. When your Shadow Word: Death kills an enemy, you have a $s3% chance to summon a Tentacle of Madness.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


deathspeaker_200252 = spell(
    id=200252,
    name='Deathspeaker',
    school=School.NORMAL,
    attributes=464,
    ...
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=29, implicit_target_a=1, apply_aura=AuraType.ADD_PCT_MODIFIER, misc_value=SpellModOp.DAMAGE),
        Effect(type=EffectType.APPLY_AURA, base_points=44, implicit_target_a=1, apply_aura=AuraType.DUMMY),
        Effect(type=EffectType.APPLY_AURA, base_points=99, implicit_target_a=1, apply_aura=AuraType.DUMMY),
    ],
    spell_icon_id=1980,
    notes='Priest Shadow rework (SHADOW.md (4,1)): Deathspeaker rank 3 - see deathspeaker_200250.',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.SWD, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Shadow Word: Death deals $s1% additional damage. When you take damage from your own Shadow Word: Death, you have a $s2% chance to summon a Tentacle of Madness. When your Shadow Word: Death kills an enemy, you have a $s3% chance to summon a Tentacle of Madness.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


lash_of_insanity_200253 = spell(
    id=200253,
    ...
    spell_icon_id=548,
    notes='Priest Shadow rework (SHADOW.md (5,0) / ID map): NEW talent, repurposes 1908 (was Psychic '
          'Horror at 8,0). eff1 SPELLMOD_DAMAGE +5/10/15% and the procs_on() below are both scoped '
          '(MIND_FLAY_TICK, 0, MIND_FLAY) - BOTH dwords are load-bearing, do not trim either. '
          'dword3 MIND_FLAY covers the channel row 15407 and the tentacle\'s own clone 200246, which '
          'the SpellMod is meant to reach via GetSpellModOwner (deliberate, PLAN §4.4). dword1 '
          'MIND_FLAY_TICK covers 58381 - the helper this server\'s 15407 fires from its '
          'PERIODIC_TRIGGER_SPELL_WITH_VALUE effect, which is the SpellInfo that actually carries '
          'the player\'s own Mind Flay damage and carries dword1 only. Scoped to dword3 alone (as '
          'this row originally shipped) neither half of the talent did anything to the priest\'s own '
          'Mind Flay: the SpellMod\'s IsAffectedOnSpell(58381) failed and the proc never matched. '
          'For the same reason procs_on() needs DONE_SPELL_MAGIC_DMG_CLASS_NEG as well as '
          'DONE_PERIODIC - 58381 is cast per tick as an ordinary triggered spell, so Spell.cpp gives '
          'it the magic-damage proc flag, not the periodic one; DONE_PERIODIC is kept because the '
          'tentacle clone 200246 IS a real periodic-damage aura. eff2 DUMMY = tentacle spawn roll '
          'chance 5/10/15%. Tooltip capstone line (final rank only): Mastery bonus to personal Mind '
          'Flay only - Priest::ApplyDoneDamagePctMods applies it inside the dword1 Mind Flay branch, '
          'which a tentacle never enters, so it cannot double-dip.',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.MIND_FLAY_TICK, 'EffectSpellClassMaskA_3': _masks.MIND_FLAY, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Mind Flay deals $s1% additional damage. Your Mind Flay damage has a $s2% chance to summon a Tentacle of Madness.\n\n|cFF9D9D9DCapstone Bonus: Your Mind Flay damage is increased by your Mastery. This does not affect Tentacles of Madness.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)
procs_on(lash_of_insanity_200253, PROC_FLAG_DONE_PERIODIC | PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG,
         family_name=6, family_mask=(_masks.MIND_FLAY_TICK, 0, _masks.MIND_FLAY),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=5)
scripted_by(lash_of_insanity_200253, 'spell_pri_lash_of_insanity')


lash_of_insanity_200254 = spell(
    id=200254,
    name='Lash of Insanity',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=548,
    notes='Priest Shadow rework (SHADOW.md (5,0)): Lash of Insanity rank 2 - see lash_of_insanity_200253.',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.MIND_FLAY_TICK, 'EffectSpellClassMaskA_3': _masks.MIND_FLAY, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Mind Flay deals $s1% additional damage. Your Mind Flay damage has a $s2% chance to summon a Tentacle of Madness.\n\n|cFF9D9D9DCapstone Bonus: Your Mind Flay damage is increased by your Mastery. This does not affect Tentacles of Madness.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)
procs_on(lash_of_insanity_200254, PROC_FLAG_DONE_PERIODIC | PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG,
         family_name=6, family_mask=(_masks.MIND_FLAY_TICK, 0, _masks.MIND_FLAY),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=10)
scripted_by(lash_of_insanity_200254, 'spell_pri_lash_of_insanity')


lash_of_insanity_200255 = spell(
    id=200255,
    name='Lash of Insanity',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=548,
    notes='Priest Shadow rework (SHADOW.md (5,0)): Lash of Insanity rank 3 - see lash_of_insanity_200253. '
          'Final rank only: eff3 capstone marker (literal 1) - Priest::ApplyDoneDamagePctMods checks '
          'HasAura(200255) on the caster (Player only) to grant +Mastery% to personal Mind Flay '
          '(matched on the dword1 MIND_FLAY_TICK bit, i.e. the 58381 tick - see '
          'lash_of_insanity_200253\'s notes).',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.MIND_FLAY_TICK, 'EffectSpellClassMaskA_3': _masks.MIND_FLAY, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Mind Flay deals $s1% additional damage. Your Mind Flay damage has a $s2% chance to summon a Tentacle of Madness.\n\nCapstone Bonus: Your Mind Flay damage is increased by your Mastery. This does not affect Tentacles of Madness.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)
procs_on(lash_of_insanity_200255, PROC_FLAG_DONE_PERIODIC | PROC_FLAG_DONE_SPELL_MAGIC_DMG_CLASS_NEG,
         family_name=6, family_mask=(_masks.MIND_FLAY_TICK, 0, _masks.MIND_FLAY),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=15)
scripted_by(lash_of_insanity_200255, 'spell_pri_lash_of_insanity')


insatiable_thirst_200256 = spell(
    id=200256,
    ...
    spell_icon_id=2213,
    notes='Priest Shadow rework (SHADOW.md (5,2) / ID map): NEW talent, repurposes 2027 (was Improved '
          'Spirit Tap at 0,1). eff1 SPELLMOD_DAMAGE scoped to Vampiric Touch (dword2 VT) +6/12/20%. '
          "eff2 DUMMY = the tentacle's extra VT-target damage bonus 5/10/15% (read in the tentacle "
          "damage path, Priest::ApplyDoneDamagePctMods, stacking additively with the tentacle's own "
          'base 10% VT bonus per design doc §7).',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.VT, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Vampiric Touch deals $s1% additional damage. Your Tentacles of Madness deal an additional $s2% damage to targets afflicted by your Vampiric Touch.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


insatiable_thirst_200257 = spell(
    id=200257,
    name='Insatiable Thirst',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=2213,
    notes='Priest Shadow rework (SHADOW.md (5,2)): Insatiable Thirst rank 2 - see insatiable_thirst_200256.',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.VT, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Vampiric Touch deals $s1% additional damage. Your Tentacles of Madness deal an additional $s2% damage to targets afflicted by your Vampiric Touch.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


insatiable_thirst_200258 = spell(
    id=200258,
    name='Insatiable Thirst',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=2213,
    notes='Priest Shadow rework (SHADOW.md (5,2)): Insatiable Thirst rank 3 - see insatiable_thirst_200256.',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.VT, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Vampiric Touch deals $s1% additional damage. Your Tentacles of Madness deal an additional $s2% damage to targets afflicted by your Vampiric Touch.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


vulnerability_200259 = spell(
    id=200259,
    name='Vulnerability',
    school=School.SHADOW,
    dispel=DispelType.NONE,
    ...
    spell_icon_id=2179,
    notes='Priest Shadow rework (SHADOW.md (6,2) / ID map): Shadow Power capstone debuff, a marker '
          'only - the +8% Magic-damage-taken bonus is applied in Priest::ApplyDoneDamagePctMods '
          '(WP-B, caster-specific: victim has 200259 from THIS caster or the tentacle\'s owner), so '
          "this row carries no real aura effect value. Reuses Shadow Power's own icon (2179).",
    raw_overrides={'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Magic damage taken from the caster increased by 8%.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Magic damage taken from the caster increased.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)


void_touched_mind_200260 = spell(
    id=200260,
    name='Void-touched Mind',
    school=School.NORMAL,
    attributes=464,
    cast_time_ms=0,
    effects=[
    ...
    spell_icon_id=211,
    notes='Priest Shadow rework (SHADOW.md (7,2) / ID map): NEW talent, repurposes 541 (was Silence at '
          '4,0 - Silence itself moves to the base kit, see priest_spells.py). eff1 SPELLMOD_DAMAGE and '
          'eff2 SPELLMOD_COST, both scoped to Mind Blast|Mind Sear|Mind Flay (all three dwords). '
          'The two are deliberately NOT scoped identically: Mind Flay and Mind Sear each deal '
          'their damage from a SEPARATE row that the cast row triggers per tick (15407 -> 58381, '
          '48045 -> 49821), and those tick rows carry their own family bits (MIND_FLAY_TICK in '
          'dword1, MIND_SEAR_TICK in dword2), so the DAMAGE half (letter A) has to list the tick '
          'bits too or it silently matches nothing for either spell. The COST half (letter B) '
          'stays on the cast bits alone - mana is paid on 15407/48045, and neither tick row is '
          'ever cast by the player. Same split as lash_of_insanity_200253. '
          "Reuses Silence's own icon (211) since this talent inherited its slot. Tooltip capstone "
          "line (final rank only): Mind Blast extends Voidform's (200140) duration by 1 sec - the 1 "
          'second value is load-bearing per design doc §7 (3 sec would make Void Eruption permanent).',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.MIND_BLAST | _masks.MIND_FLAY_TICK, 'EffectSpellClassMaskA_2': _masks.MIND_SEAR | _masks.MIND_SEAR_TICK, 'EffectSpellClassMaskA_3': _masks.MIND_FLAY, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EffectSpellClassMaskB_2': _masks.MIND_SEAR, 'EffectSpellClassMaskB_3': _masks.MIND_FLAY, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Mind Blast, Mind Flay and Mind Sear spells deal $s1% more damage and cost $s2% less mana.\n\n|cFF9D9D9DCapstone Bonus: Your Mind Blast extends the duration of Void Eruption by 1 second.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


void_touched_mind_200261 = spell(
    id=200261,
    name='Void-touched Mind',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=211,
    notes='Priest Shadow rework (SHADOW.md (7,2)): Void-touched Mind rank 2 - see void_touched_mind_200260.',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.MIND_BLAST | _masks.MIND_FLAY_TICK, 'EffectSpellClassMaskA_2': _masks.MIND_SEAR | _masks.MIND_SEAR_TICK, 'EffectSpellClassMaskA_3': _masks.MIND_FLAY, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EffectSpellClassMaskB_2': _masks.MIND_SEAR, 'EffectSpellClassMaskB_3': _masks.MIND_FLAY, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Mind Blast, Mind Flay and Mind Sear spells deal $s1% more damage and cost $s2% less mana.\n\n|cFF9D9D9DCapstone Bonus: Your Mind Blast extends the duration of Void Eruption by 1 second.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


void_touched_mind_200262 = spell(
    id=200262,
    name='Void-touched Mind',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=211,
    notes='Priest Shadow rework (SHADOW.md (7,2)): Void-touched Mind rank 3 - see void_touched_mind_200260. '
          'Final rank only: eff3 capstone marker (literal 1) - spell_pri_mind_blast_shadow (WP-B) '
          'checks HasAura(200262) to extend Voidform by 1 sec on Mind Blast cast.',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.MIND_BLAST | _masks.MIND_FLAY_TICK, 'EffectSpellClassMaskA_2': _masks.MIND_SEAR | _masks.MIND_SEAR_TICK, 'EffectSpellClassMaskA_3': _masks.MIND_FLAY, 'EffectSpellClassMaskB_1': _masks.MIND_BLAST, 'EffectSpellClassMaskB_2': _masks.MIND_SEAR, 'EffectSpellClassMaskB_3': _masks.MIND_FLAY, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Mind Blast, Mind Flay and Mind Sear spells deal $s1% more damage and cost $s2% less mana.\n\nCapstone Bonus: Your Mind Blast extends the duration of Void Eruption by 1 second.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


writhing_agony_200263 = spell(
    id=200263,
    name='Writhing Agony',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=234,
    notes='Priest Shadow rework (SHADOW.md (8,0) / ID map): NEW talent, minted id 60023. eff1 '
          'SPELLMOD_DAMAGE scoped to Shadow Word: Pain (dword1 SWP) +6/12/20%. eff2 DUMMY = tentacle '
          'spawn roll chance 5/10/15% (procs_on below). Tooltip capstone line (final rank only): '
          "150% of Mastery to the tentacle's damage only (spell_pri_writhing_agony marker read in "
          'the tentacle damage path, Priest::ApplyDoneDamagePctMods).',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.SWP, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Shadow Word: Pain deals $s1% additional damage. Your Shadow Word: Pain damage has a $s2% chance to summon a Tentacle of Madness.\n\n|cFF9D9D9DCapstone Bonus: Your Tentacles of Madness deal damage increased by 150% of your Mastery.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)
procs_on(writhing_agony_200263, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.SWP, 0, 0),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=5)
scripted_by(writhing_agony_200263, 'spell_pri_writhing_agony')


writhing_agony_200264 = spell(
    id=200264,
    name='Writhing Agony',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=234,
    notes='Priest Shadow rework (SHADOW.md (8,0)): Writhing Agony rank 2 - see writhing_agony_200263.',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.SWP, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Shadow Word: Pain deals $s1% additional damage. Your Shadow Word: Pain damage has a $s2% chance to summon a Tentacle of Madness.\n\n|cFF9D9D9DCapstone Bonus: Your Tentacles of Madness deal damage increased by 150% of your Mastery.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)
procs_on(writhing_agony_200264, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.SWP, 0, 0),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=10)
scripted_by(writhing_agony_200264, 'spell_pri_writhing_agony')


writhing_agony_200265 = spell(
    id=200265,
    name='Writhing Agony',
    school=School.NORMAL,
    attributes=464,
    ...
    spell_icon_id=234,
    notes='Priest Shadow rework (SHADOW.md (8,0)): Writhing Agony rank 3 - see writhing_agony_200263. '
          'Final rank only: eff3 capstone marker (literal 1) - read in the tentacle damage path '
          '(Priest::ApplyDoneDamagePctMods, HasAura(200265) on the owner) for the x1.5 Mastery bonus.',
    raw_overrides={'EffectSpellClassMaskA_1': _masks.SWP, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Your Shadow Word: Pain deals $s1% additional damage. Your Shadow Word: Pain damage has a $s2% chance to summon a Tentacle of Madness.\n\nCapstone Bonus: Your Tentacles of Madness deal damage increased by 150% of your Mastery.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)
procs_on(writhing_agony_200265, PROC_FLAG_DONE_PERIODIC, family_name=6, family_mask=(_masks.SWP, 0, 0),
         spell_phase_mask=PROC_SPELL_PHASE_HIT, chance=15)
scripted_by(writhing_agony_200265, 'spell_pri_writhing_agony')


dissolving_shadows_200266 = spell(
    id=200266,
    name='Dissolving Shadows',
    school=School.NORMAL,
    attributes=464,
    cast_time_ms=0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-3, implicit_target_a=1, apply_aura=AuraType.ADD_FLAT_MODIFIER, misc_value=SpellModOp.EFFECT1),
        Effect(type=EffectType.APPLY_AURA, base_points=-20001, implicit_target_a=1, apply_aura=AuraType.ADD_FLAT_MODIFIER, misc_value=SpellModOp.COOLDOWN),
    ],
    spell_icon_id=2875,
    notes='Priest Shadow rework (SHADOW.md (8,3) / ID map): NEW talent, minted id 60024. eff1 '
          "SPELLMOD_EFFECT1 (misc=3) flat -2/-4/-6, scoped to Dispersion (dword2, letter A) - adds "
          "directly to Dispersion's own Effect_1 (the -90% MOD_DAMAGE_PERCENT_TAKEN, confirmed by "
          'reading dispersion_47585 in priest_spells.py: its damage-taken aura sits on effect index '
          '1/letter A) for -92/-94/-96% total. eff2 SPELLMOD_COOLDOWN flat -20000/-40000/-60000 ms, '
          "same scope (letter B). Reuses Dispersion's own icon (2875). Tooltip capstone line (final "
          'rank only): +25% periodic spell damage while in Dispersion (spell_pri_dissolving_shadows '
          'marker via Priest::ApplyDoneDamagePctMods, damagetype==DOT and caster HasAura(47585) and '
          'HasAura(200268)).',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.DISPERSION, 'EffectSpellClassMaskB_2': _masks.DISPERSION, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the damage reduction of Dispersion by an additional $s1%. Reduces the cooldown of Dispersion by $/1000;s2 seconds.\n\n|cFF9D9D9DCapstone Bonus: Your periodic spell damage is increased by 25% while you are in Dispersion.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


dissolving_shadows_200267 = spell(
    id=200267,
    name='Dissolving Shadows',
    school=School.NORMAL,
    attributes=464,
    cast_time_ms=0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-5, implicit_target_a=1, apply_aura=AuraType.ADD_FLAT_MODIFIER, misc_value=SpellModOp.EFFECT1),
        Effect(type=EffectType.APPLY_AURA, base_points=-40001, implicit_target_a=1, apply_aura=AuraType.ADD_FLAT_MODIFIER, misc_value=SpellModOp.COOLDOWN),
    ],
    spell_icon_id=2875,
    notes='Priest Shadow rework (SHADOW.md (8,3)): Dissolving Shadows rank 2 - see dissolving_shadows_200266.',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.DISPERSION, 'EffectSpellClassMaskB_2': _masks.DISPERSION, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the damage reduction of Dispersion by an additional $s1%. Reduces the cooldown of Dispersion by $/1000;s2 seconds.\n\n|cFF9D9D9DCapstone Bonus: Your periodic spell damage is increased by 25% while you are in Dispersion.|r', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0},
)


dissolving_shadows_200268 = spell(
    id=200268,
    name='Dissolving Shadows',
    school=School.NORMAL,
    attributes=464,
    cast_time_ms=0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=-7, implicit_target_a=1, apply_aura=AuraType.ADD_FLAT_MODIFIER, misc_value=SpellModOp.EFFECT1),
    ...
    spell_icon_id=2875,
    notes='Priest Shadow rework (SHADOW.md (8,3)): Dissolving Shadows rank 3 - see dissolving_shadows_200266. '
          'Final rank only: eff3 capstone marker (literal 1) - Priest::ApplyDoneDamagePctMods checks '
          'HasAura(200268) on the caster for the +25% periodic-damage-in-Dispersion bonus.',
    raw_overrides={'EffectSpellClassMaskA_2': _masks.DISPERSION, 'EffectSpellClassMaskB_2': _masks.DISPERSION, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Increases the damage reduction of Dispersion by an additional $s1%. Reduces the cooldown of Dispersion by $/1000;s2 seconds.\n\nCapstone Bonus: Your periodic spell damage is increased by 25% while you are in Dispersion.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0, 'EffectChainAmplitude_2': 1.0, 'EffectChainAmplitude_3': 1.0},
)


sundered_mind_200270 = spell(
    id=200270,
    name='Sundered Mind',
    school=School.SHADOW,
    dispel=DispelType.NONE,
    ...
    duration_ms=8000,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=0, die_sides=0, implicit_target_a=1, apply_aura=AuraType.PERIODIC_DUMMY, amplitude=1000),
    ],
    spell_icon_id=1980,
    notes='Priest Shadow rework (SHADOW.md (10,1) / ID map): Surrender to Madness\'s aftermath debuff. '
          'PERIODIC_DUMMY (226), 8 ticks over 8 s - the raw 20%-max-health-per-tick damage is dealt '
          'in spell_pri_sundered_mind (WP-B, OnEffectPeriodic -> Unit::DealDamage directly, bypassing '
          'CalcAbsorbResist/SpellDamageBonusTaken entirely so PW:S/Shadowform/Dispersion/Versatility '
          'cannot reduce it - design doc §4.6), not a DBC damage effect. Attributes |= '
          'SPELL_ATTR0_NO_AURA_CANCEL (0x80000000, confirmed against SharedDefines.h\'s SpellAttr0 '
          'enum - this is the base Attributes field, NOT AttributesEx) so the priest cannot right-'
          'click-cancel it. ALSO sets SPELL_ATTR0_AURA_IS_DEBUFF (0x04000000, same field) - verified '
          'via SpellInfo::_IsPositiveEffect (SpellInfo.cpp:3069): a self-targeted PERIODIC_DUMMY aura '
          'has no automatic negative signal in that function (PERIODIC_DUMMY is not one of the aura '
          'types the switch special-cases, unlike PERIODIC_DAMAGE which is auto-negative-on-self) - '
          'without this explicit flag the engine could misclassify Sundered Mind as a *positive* '
          'aura, which would be wrong for a "cannot cancel, deals lethal damage" debuff. This is the '
          "task's flagged verify-rather-than-assume item, resolved with evidence rather than left "
          'to the default.',
    raw_overrides={'Attributes': 2214592512, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Taking damage equal to 20% of your maximum health every 1 sec. This damage cannot be reduced, absorbed or prevented by any effect, including Dispersion.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Taking massive periodic Shadow damage.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)
scripted_by(sundered_mind_200270, 'spell_pri_sundered_mind')


madness_200271 = spell(
    id=200271,
    name='Madness',
    school=School.SHADOW,
    cast_time_ms=0,
    ...
    spell_icon_id=234,
    notes='Priest Shadow rework (SHADOW.md "4.3 Madness" / ID map): the visible Madness resource '
          'display - a stacking aura, CumulativeAura=250 (visible cap, = floor(internal 500/2) per PLAN '
          "§1's display transform). Priest::AddMadness (WP-B) sets the live stack count directly "
          '(Aura::SetStackAmount, not this row\'s base_points) and refreshes the duration on every '
          'gain - "one stacking aura, 30 sec duration, refreshing on every stack gained" per design '
          'doc §4.3, not per-stack independent expiry. AttributesEx4=4 '
          '(SPELL_ATTR4_AURA_EXPIRES_OFFLINE): Player::_LoadAuras only deducts offline time from '
          'auras that are negative or carry this attribute, and this one is a positive self-buff - '
          'without it a saved Madness aura returns at its full remaining duration however long the '
          'character was logged off, while its backing internal value (PriestMechanics.cpp) is '
          'cleared on logout, leaving a phantom stack display. With it the 30 s ticks down offline, '
          'so any logout longer than the duration comes back clean and a shorter one self-heals.',
    raw_overrides={'AttributesEx4': 4, 'CumulativeAura': 250, 'SpellClassSet': 6, 'Name_Lang_Mask': 16712190, 'NameSubtext_Lang_Mask': 16712190, 'NameSubtext_Lang_enUS': '', 'Description_Lang_Mask': 16712190, 'Description_Lang_enUS': 'Madness accumulated from Shadow damage.', 'AuraDescription_Lang_Mask': 16712190, 'AuraDescription_Lang_enUS': 'Madness.', 'EquippedItemClass': -1, 'ProcChance': 101, 'RangeIndex': 1, 'SpellPriority': 50, 'EffectChainAmplitude_1': 1.0},
)
scripted_by(madness_200271, 'spell_pri_madness')


tentacle_scaling_200272 = spell(
    id=200272,
    name='Tentacle of Madness: Scaling',
    school=School.SHADOW,
    cast_time_ms=0,
    effects=[
        Effect(type=EffectType.APPLY_AURA, base_points=0, die_sides=0, implicit_target_a=1, apply_aura=AuraType.MOD_DAMAGE_DONE, misc_value=int(School.SHADOW)),
        Effect(type=EffectType.APPLY_AURA, base_points=0, die_sides=0, implicit_target_a=1, apply_aura=AuraType.MOD_CRIT_PCT),
        Effect(type=EffectType.APPLY_AURA, base_points=0, die_sides=0, implicit_target_a=1, apply_aura=AuraType.HASTE_ALL),
    ],
    spell_icon_id=2296,
    notes="Priest Shadow rework (SHADOW.md (3,0) / ID map): the tentacle's own owner-stat snapshot "
          'aura, cast on itself at summon by npc_pri_tentacle_of_madness (WP-B). WP-C reconciliation '
          "(2026-09-22): WP-B's spell_pri_tentacle_scaling (spell_priest_shadow.cpp) registers "
          'DoEffectCalcAmount handlers filtered on real AuraTypes (MOD_DAMAGE_DONE/MOD_CRIT_PCT/'
          'HASTE_ALL - same shape spell_pri_shadowfiend_scaling in spell_priest.cpp already uses '
          'against a stock spell whose DBC data already carries those types), matching the design '
          "doc sec 4.1's \"reads spell power, haste, crit\" via the engine's own damage/crit/haste "
          'pipeline rather than a manual multiplier in ApplyDoneDamagePctMods. The original WP-A '
          'pass declared all three effects as plain DUMMY (a reasonable guess absent visibility '
          "into WP-B's concurrent C++, flagged explicitly in both agents' handoff reports as a "
          "coordination risk to check here) - those calc-amount handlers would never have fired "
          'against a DUMMY effect, silently leaving the tentacle at its guardian-default (near-zero) '
          'spell power. base_points=0/die_sides=0 on all three since DoEffectCalcAmount overrides '
          'the amount unconditionally at runtime; misc_value=32 (School.SHADOW) on effect 1 scopes '
          "the spell-power grant to Shadow damage, matching CalculateSpellPower's own "
          'SPELL_SCHOOL_MASK_SHADOW read. Mastery/Versatility have no native per-Unit field on this '
          "server (PLAN sec 3.8/6's custom stats) and are captured separately into "
          "Priest::TentacleSnapshot's manual backing map by the same AuraScript's OnEffectApply "
          '(EFFECT_0/SPELL_AURA_ANY, unaffected by this fix either way), read later from the '
          "tentacle's own damage path in Priest::ApplyDoneDamagePctMods.",
