...
from lib import build, dbcfile, dbcfmt, patch_out, source, sql_out, state  # noqa: E402
...
def main() -> int:
    ...
    spell_rows = [build.build_spell_row(e, reuse) for e in spell_entries]
    talent_rows = [build.build_talent_row(e) for e in talents["talents"]]
    talenttab_rows = [build.build_talenttab_row(e) for e in talents["tabs"]]

    # -- pending SQL: our reserved ranges only, in every touched table --
    blocks = [
        (dbcfmt.SPELL, ids_cfg["spell"], spell_rows),
        (dbcfmt.TALENT, ids_cfg["talent"], talent_rows),
        (dbcfmt.TALENTTAB, ids_cfg["talenttab"], talenttab_rows),
    ]
    for name, table in SECONDARY_TABLES.items():
    ...
    header = (
        ...
        "-- Source of truth: apps/dbc-tools/source/{spells.csv,talents.yaml}.\n"
