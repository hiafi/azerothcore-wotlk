        ...
        if (m_originalCaster)
        {
            ...
            if (m_spellAura)
            {
                ...
                else
                {
                    ...
                    // Haste no longer shrinks periodic-aura duration -- see Aura::RefreshDuration
                    // and AuraEffect::GetFinalTickBonusMultiplier.
...
SpellCastResult Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura)
{
    ...
    // Fire Mage rework (docs/reworks/fire-mage-rework.md, Blazing Speed capstone) - "you can cast
    // non-channeled Fire spells while moving" while the escape buff is up. Single extra clause
    // rather than a class-specific branch inline; Mage::CanCastWhileMoving is the one call site.
    if ((m_spellInfo->IsChanneled() || m_casttime) && unitCaster && unitCaster->IsPlayer() && unitCaster->isMoving() && m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_MOVEMENT && !IsTriggered() &&
        !Mage::CanCastWhileMoving(unitCaster, m_spellInfo))
    {
